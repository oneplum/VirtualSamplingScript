...
lf = pl.scan_parquet(BASE_DIR / "data" / "parquet")
...
COL_META = {
    ...
    "method": {
        "label": "True Sampling Method",
        "values": METHOD_LABELS,
        "order": METHOD_ORDER
    },
    "lighting_enabled": {
        "label": "Illumination",
        "values": {True: "Lighting enabled", False: "Lighting disabled"},
        "order": [False, True]
    },
    "virtual_sampling_method": {
        "label": "Virtual Sampling Method",
...
def update_main_chart(
    ...
    for col_name, filter_val in zip(FILTER_COLS, filter_vals):
        ...
        filters.append(pl.col(col_name).is_in(filter_val))


    if filters:
        flf = lf.filter(pl.all_horizontal(filters))
    else:
        flf = lf
    ...
    flf = flf.unpivot(
        on=METRIC_COLS,
        index=FILTER_COLS,
        variable_name="band_idx",
        value_name="blti"
    )

    groupby = ["method", "virtual_sampling_method", "band_idx"]

    agg = (
        flf.group_by(groupby)
        .agg(pl.col("blti").mean())
        .select(["method", "virtual_sampling_method", "band_idx", "blti"])
    )
